...
type ReplyRelation struct {
	FromPost int64
	ToPost   int64
	Type     string // "direct", "quote", "mention"
}
...
func (pa *PostAnalyzer) ParsePost(postNo int64, htmlComment string) (*ParsedPost, error) {
	...
	// Look for additional mentions in plain text (>>123456 without links)
	mentionRegex := regexp.MustCompile(`&gt;&gt;(\d+)`)
	mentionMatches := mentionRegex.FindAllStringSubmatch(htmlComment, -1)
	for _, match := range mentionMatches {
		if len(match) >= 2 {
			if mention, err := strconv.ParseInt(match[1], 10, 64); err == nil {
				// Only add if not already in reply links
				if !contains(parsed.ReplyLinks, mention) {
...
// stripHTML removes HTML tags and cleans up text
func (pa *PostAnalyzer) stripHTML(htmlText string) string {
	...
	// Extract text preserving quotes and reply links
